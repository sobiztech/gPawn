                    ...
                    <table class="table table-bordered text-nowrap border-bottom" id="basic-datatable">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <a class="btn btn-blue edit" title="Edit" data-id="{{ $row->id }}" data-customer_id="{{ $row->customer_id }}" data-black_list_type_id="{{ $row->black_list_type_id }}">
                                        <i style="color:rgb(226, 210, 210);cursor: pointer" class="fa fa-edit"></i>
                                    </a>

                                    <a class="btn btn-red delete" title="Delete" data-url="{{ route('blacklist.delete') }}" data-id="{{ $row->id }}">
                                        <i style="color:rgb(226, 210, 210);cursor: pointer" class="fa fa-trash-o"></i>
                                    </a>
                                </td>
                            </tr>
                            @endforeach
            ...
            <div class="modal-body">
                ...
                <form method="POST" action="{{ route('blacklist.store') }}">
                    ...
                    <div class="row">
                                ...
                                <div>
                                    <select class="form-control select2-show-search form-select" required name="customer_id" id="customer_id">
                                        ...
                                        <option value="{{ $item->id }}" {{ old('customer_id') == $item->id ? 'selected' : '' }}>
                                            {{ $item->customer_first_name }}/ {{ $item->phone_number }}
                                        </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <label>Black List Type<span class="text-danger">*</span></label>
                                <div>
                                    <select class="form-select" required name="black_list_type_id" id="black_list_type_id">
                                        <!-- <option selected disabled value="">Choose...</option> -->
                                        @foreach ($blackListTypes as $item)
                                        <option selected value="{{ $item->id }}" {{ (old('black_list_type_id') == $item->id) ? 'selected' : '' }}>{{ $item->black_list_type_name }}</option>
                                        @endforeach
                                    </select>
                                    <p style="color:Tomato"> @error('black_list_type_id'){{ $message }} @enderror</p>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            swal({
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        ...
                        $.ajax({
                            url: '/blacklist/deleteurl',
                            method: 'post',
                            data: {
                                id: id
                            },
                            success: function(res) {
                                swal('Poof! Remove Customer From Black List!', {
                                    icon: 'success',
                                    timer: 1000,
                                });
                                location.reload();
